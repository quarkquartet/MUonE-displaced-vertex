...
result_path = "./reach_10sigma_m2.csv"
...
with open(result_path, "w") as f:
    ...
    for mA in mass_list:
        ...
        for coupling in couplings:
            selection=((LHE.conv_data[:,2] <= Radius) & (LHE.conv_data[:,6] <= Radius) & (LHE.conv_data[:,10] <= Radius)& (LHE.conv_data[:,13] >= E_cut) & (LHE.conv_data[:,15] >= E_cut) & (LHE.conv_data[:,4] >= E_cut) & (LHE.conv_data[:,8] >= E_cut) & (LHE.conv_data[:,16] >= angle_cut))
            ...
            sensitivity = x_section_mass * total_fixed_target_luminosity * coupling**2 * efficiency
