...
open_angle_cut = 1.0 / 1000.0  # input mrad, change to rad
L_min = 0.02
L_max = ((15.0) - 0.5) / 100.0  # input cm, change to m
...
def probability(momentum, theta, mass, g_e):
    ...
    if L_max * np.tan(theta) <= Radius:
        ...
        if (1.0 + L_max) * np.tan(theta) <= Radius:
            next_min = 1.0 + L_min
            next_max = 1.0 + L_max
            L_prob += np.exp(-next_min / travel_distance) * (
                1.0 - np.exp((next_min - next_max) / travel_distance)
            )
    else:
        L_prob = 0.0
    return L_prob
...
def get_sensitivity(mass, g_e, x_section):
    ...
    for i in range(len(X_list)):
        ...
        if open_angle >= open_angle_cut and E >= E_cut:
